    ...
    def main_callback(self):
        ...
        if self.state == 'takeoff':
            if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_LOITER: # Take off 끝나면 Auto Loiter로 바뀜
                self.publish_vehicle_command(VehicleCommand.VEHICLE_CMD_DO_VTOL_TRANSITION, param1 = 1.0) # transition to Fixed-wing mode
        ...
        if self.state == 'Flying':
            ...
            if np.linalg.norm(delta) < 10: # setpoint 가까워지면
                print("도착!")
                self.publish_vehicle_command(VehicleCommand.VEHICLE_CMD_DO_VTOL_TRANSITION, param1=0.0) # transition to Multi-rotor mode
                self.state = 'Landing'
