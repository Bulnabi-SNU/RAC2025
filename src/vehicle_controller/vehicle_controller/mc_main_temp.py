...
class MissionState(Enum):
    ...
    DROPTAG_HOVER = "DROPTAG_HOVER" #
    DROP_TAG_ASCEND = "DROP_TAG_ASCEND"
    
    LANDING_TAG_DESCEND = "LANDING_TAG_DESCEND" #
...
class MissionController(PX4BaseController):
    ...
    def _load_parameters(self):
        ...
        params = [
            ...
            ('landing_detection_required', 10),   # <-- new ROS parameter
            ('do_logging', True),
        ]
    ...
    def main_loop(self):
        ...
        state_handlers = {
            ...
            MissionState.START_ASCEND: lambda: self._handle_descend_ascend(MissionState.CASUALTY_TRACK, self.mission_altitude),
            ...
            MissionState.CASUALTY_HOVER: self._handle_casualty_hover,
            MissionState.CASUALTY_ASCEND: lambda: self._handle_descend_ascend(MissionState.DROP_TAG_TRACK, self.mission_altitude), 
            MissionState.DROP_TAG_TRACK: lambda: self._handle_track_target(MissionState.DROP_TAG_DESCEND),
            MissionState.DROP_TAG_DESCEND: lambda: self._handle_descend_ascend(MissionState.DROPTAG_HOVER, self.gripper_altitude),
            MissionState.DROPTAG_HOVER: self._handle_droptag_hover, 
            MissionState.DROP_TAG_ASCEND: lambda: self._handle_descend_ascend(MissionState.LANDING_TAG_TRACK_5M, self.mission_altitude),

        # To joongwon: removed MOVE_TO_TARGET and LANDING_TAG_DESCEND ( with LANDING_TAG_DESCEND, drone will ascend 15m above droptag and go down to 5m at the same location)

            #MissionState.MOVE_TO_TARGET: lambda: self._handle_move_to_target(MissionState.LANDING_TAG_DESCEND, 5.0,0.0,15.0),  
            #MissionState.LANDING_TAG_DESCEND: lambda: self._handle_descend_ascend(MissionState.LANDING_TAG_TRACK_5M, 5.0), 
            MissionState.LANDING_TAG_TRACK_5M: lambda: self._handle_landing_track_to_alt(MissionState.LANDING_TAG_TRACK_1M, 5.0),
            MissionState.LANDING_TAG_TRACK_1M: lambda: self._handle_landing_track_to_alt(MissionState.LANDING_TAG_TRACK, 1.0), #changed MissionState.LAND->MissionState.LANDING_TAG_TRACK
    ...
    def param_update_callback(self, params):
        ...
        for p in params:
            ...
            elif p.name == 'landing_detection_required':
                # ensure integer count
                try:
                    self.landing_detection_required = int(p.value)
                except Exception:
                    self.get_logger().warn("landing_detection_required must be an integer; ignoring update")
    ...
    def print_current_parameters(self):
        ...
        self.get_logger().info(f"  landing_detection_required: {self.landing_detection_required}")
    ...
    def _handle_offboard_arm(self):
        ...
        if self.is_disarmed():
            ...
            # self._start_logging()
            self.state = MissionState.START_ASCEND #removed move_to_target
    ...
    def _handle_landing_track_to_alt(self, next_state: MissionState, target_altitude: float):
        """
        Minimal handler for landing tracking stages:
        - requires consecutive detections (self.landing_detection_required)
        - sets the drone_target_controller.target_altitude to requested altitude
        - uses the existing drone_target_controller.update(...) to generate the setpoint
        - transitions to next_state when arrived
        """
        # require valid detection messages
        if self.target is None or self.target.status != 0:
            self.landing_detection_count = 0
            self.get_logger().warn("No landing tag detection, waiting for CV detection")
            return

        # count consecutive detections (simple debounce)
        self.landing_detection_count += 1
        if self.landing_detection_count < self.landing_detection_required:
            self.get_logger().info(f"Landing tag detection {self.landing_detection_count}/{self.landing_detection_required}")
            return

        # enough confirmations -> start tracking toward requested altitude
        self.drone_target_controller.target_altitude = target_altitude
        ...
        if arrived:
            # reset detection count and controller, then move to next stage
            self.landing_detection_count = 0
            self.drone_target_controller.reset()
            self.state = next_state
    ...
    def _handle_casualty_hover(self):
        """hovering"""
        self.get_logger().info("Hovering above casualty")
        # TODO: Implement gripper control
        
        self.pickup_complete = True
        self.state = MissionState.CASUALTY_ASCEND

    def _handle_droptag_hover(self):
        """hovering"""
        self.get_logger().info("hovering above droptag")
        # TODO: Implement gripper control
